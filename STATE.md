...
- **Deferred hardening** — CSP still carries `'unsafe-inline'` (the app uses inline event handlers; removing it needs an app-wide handler refactor + real-browser QA). No JS bundling/minification yet (client loads ~16 raw scripts; the app is service-worker-cached so this is a first-load cost).
