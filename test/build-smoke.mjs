...
async function main() {
  ...
  // SEO content surfaces — the crawlable static pages that grow organic traffic
  // must be generated by the build (regression guard for scripts/seo/*).
  {
    // Learn articles (openings/tactics/rules/checkers) — one static page each.
    const learnPages = fs.readdirSync(path.join(DIST, 'learn')).filter((f) => f.endsWith('.html'));
    assert(learnPages.length >= 55, `expected >=55 learn pages, got ${learnPages.length}`);
    for (const f of ['en-passant-the-pawn-capture-beginners-miss.html',
                     'international-draughts-10-10-rules-and-flying-kings.html']) {
      assert(fs.existsSync(path.join(DIST, 'learn', f)), `dist/learn/${f} missing (content pipeline broke)`);
    }
    // Opening guide pages + hub.
    assert(fs.existsSync(path.join(DIST, 'openings', 'index.html')), 'dist/openings/index.html (hub) missing');
    const openingPages = fs.readdirSync(path.join(DIST, 'openings')).filter((f) => f.endsWith('.html') && f !== 'index.html');
    assert(openingPages.length >= 7, `expected >=7 opening pages, got ${openingPages.length}`);
    // Free tools + companion scripts + hub.
    for (const f of ['tools/index.html', 'tools/elo-rating-calculator.html', 'tools/elo-rating-calculator.js',
